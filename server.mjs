...
loadEnv(path.join(__dirname, '.env.example'), false);

const HOST = process.env.HOST || '127.0.0.1';
const PORT = Number(process.env.PORT || 5177);
const KB_PROJECT_ROOT = process.env.KB_PROJECT_ROOT || 'E:\\project_1\\ai-lib';
const BASIC_AUTH = process.env.BASIC_AUTH || '';
const PUBLIC_ROOT = path.join(__dirname, 'public');
const MODEL_CONFIG_LOCAL = path.join(__dirname, 'server', 'config', 'model.config.local.json');
const MODEL_CONFIG_EXAMPLE = path.join(__dirname, 'server', 'config', 'model.config.example.json');
const KB_PATH_CONFIG_LOCAL = path.join(__dirname, 'server', 'config', 'kb.paths.local.json');
const KB_PATH_CONFIG_EXAMPLE = path.join(__dirname, 'server', 'config', 'kb.paths.example.json');

const KB_PATHS = loadKbPathConfig();
const KB_ROOT = KB_PATHS.kbRoot || process.env.KB_ROOT || path.join(KB_PROJECT_ROOT, 'knowledge-base');
...
async function getImageMarkdown(workbook) {
  ...
  const exportScript = path.join(KB_PROJECT_ROOT, 'scripts', 'export-excel-anchored-md.ps1');
  const convertScript = path.join(KB_PROJECT_ROOT, 'scripts', 'convert-anchored-md-to-standard-image-md.mjs');
...
function generateImageMarkdown(anchoredPath, outputPath) {
  return new Promise((resolve, reject) => {
    const script = path.join(KB_PROJECT_ROOT, 'scripts', 'convert-anchored-md-to-standard-image-md.mjs');
...
async function getAnchoredMarkdown(workbook) {
  ...
  const script = path.join(KB_PROJECT_ROOT, 'scripts', 'export-excel-anchored-md.ps1');
...
function generateAnchoredMarkdown(originalPath, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn('powershell', args, { cwd: KB_PROJECT_ROOT, windowsHide: true });
    let stdout = '';
    let stderr = '';
    child.stdout.on('data', (chunk) => { stdout += chunk.toString(); });
    child.stderr.on('data', (chunk) => { stderr += chunk.toString(); });
...
function generateExcelPreview(originalPath) {
  return new Promise((resolve, reject) => {
    const script = path.join(KB_PROJECT_ROOT, 'scripts', 'export-excel-like-html.ps1');
...
async function loadModelConfig() {
  const file = existsSync(MODEL_CONFIG_LOCAL) ? MODEL_CONFIG_LOCAL : MODEL_CONFIG_EXAMPLE;
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === '/api/health') return sendJson(res, { ok: true, kbRoot: KB_ROOT, kbConfig: publicKbConfig(), scannedAt: docCache.scannedAt, wovenScannedAt: wovenDocCache.scannedAt });
